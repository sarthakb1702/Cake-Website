      ...
      <div className="mx-auto grid max-w-[1400px] items-center gap-14 px-5 md:px-10 lg:grid-cols-[1.05fr_1fr]">
        <div className="relative">
          ...
          <motion.h1
            ...
            className="font-display text-[clamp(2.9rem,7.4vw,5.9rem)] leading-[0.92] font-black uppercase"
          >
            Life&apos;s too
            <br />
            short to eat{" "}
            <Sparkle className="mb-3 inline h-7 w-7 text-butter md:h-9 md:w-9" />
            <br />
            <span className="text-rose">boring</span> cake
          </motion.h1>
          ...
          >
            ...
            Small-batch cakes baked at dawn, finished by hand, and delivered to your door the same
            afternoon. No shortcuts, no dry sponge, ever.
          </motion.p>
          ...
          >
            <button className="rounded-full bg-rose px-8 py-4 text-sm font-semibold text-primary-foreground shadow-badge transition-all hover:scale-105 hover:brightness-110">
              Order Now
            </button>
            <button className="rounded-full border-2 border-chocolate px-8 py-4 text-sm font-semibold text-chocolate transition-all hover:scale-105 hover:bg-chocolate hover:text-cream-white">
              Explore Flavours
            </button>
          </motion.div>
        </div>
        ...
        <div className="relative">
          ...
          <motion.img
            ...
            src={slide.image}
            alt={`${slide.name} artisan cake by Crumb & Co.`}
            loading="eager"
          ...
          >
            <p className="font-display text-xl font-bold">{slide.name}</p>
            <p className="mt-1.5 text-xs leading-relaxed text-muted-foreground">{slide.note}</p>
            <motion.div
              animate={{ y: [0, -7, 0] }}
              transition={{ duration: 2.6, repeat: Infinity, ease: "easeInOut" }}
              className="absolute -top-7 -right-6 flex h-16 w-16 items-center justify-center rounded-full bg-rose font-display text-lg font-black text-primary-foreground shadow-badge"
            >
              {slide.price}
            </motion.div>
          </motion.div>
          ...
          <div className="relative z-20 mt-6 flex items-center justify-center gap-4">
            <button
              ...
              className="flex h-11 w-11 items-center justify-center rounded-full border border-border bg-card transition-transform hover:scale-110"
            >
            ...
            <div className="flex gap-2">
              {heroSlides.map((s, i) => (
                <button
                  ...
                  className={
                    i === index
                      ? "h-2 w-7 rounded-full bg-rose transition-all"
